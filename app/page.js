...
const initialMarketplaceListings = [
  ...
  },
  {
    ...
    verifiedCert: 'CERT-FG-771203',
    date: 'Yesterday',
  },
  {
    id: 'MKT-103',
    title: '1999 Base Mewtwo Holo #10',
    seller: 'VintageSlabs_US',
    grade: 'PSA MINT 9',
    centering: '55/45',
    corners: '9.0',
    edges: '9.5',
...
export default function Home() {
  const [activeTab, setActiveTab] = useState('scanner'); // 'scanner' | 'marketplace'
  ...
  // Marketplace State
  const [marketplace, setMarketplace] = useState(initialMarketplaceListings);
  ...
  const runScan = async () => {
    ...
    try {
      ...
      console.error(err);
      alert('Inspection failed. Please check network or try again.');
    } finally {
      setIsScanning(false);
      setScanPhase('');
    }
  ...
  const handleListToMarketplace = () => {
    ...
    const newListing = {
      ...
      askingPrice: Number(listPrice) || 50,
      marketVal: report.gradedVal,
      image: frontImage || 'https://images.pokemontcg.io/pgo/11_hires.png',
      verifiedCert: `CERT-FG-${Math.floor(100000 + Math.random() * 900000)}`,
      date: 'Just now',
    };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-10 font-sans selection:bg-cyan-500 selection:text-white relative">
        ...
        <div>
          <div className="flex items-center gap-2">
            <span className="text-[11px] font-bold tracking-wider text-cyan-400 bg-cyan-950/60 border border-cyan-800/50 px-2.5 py-0.5 rounded-full uppercase">
              Fricks Pre-Grade Pro & Marketplace
            </span>
          ...
          <h1 className="text-2xl md:text-3xl font-extrabold mt-2 text-white tracking-tight">
            AI Card Inspector & Verified Marketplace
          </h1>
          <p className="text-slate-400 text-sm mt-1">
            Pre-grade cards with optical AI and trade verified raw cards with certified centering reports.
          </p>
        </div>
          ...
          <section className="lg:col-span-2 space-y-6">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 md:p-6 shadow-xl">
                  ...
                  {frontImage ? (
                    <div className="relative w-full h-[250px] flex items-center justify-center rounded-xl overflow-hidden">
                      ...
                      <button
                        ...
                        className="absolute top-1 right-1 z-50 bg-red-600 text-white rounded-full w-7 h-7 flex items-center justify-center text-xs shadow-lg cursor-pointer"
                      >
                        ✕
                      </button>
                    </div>
                    ...
                    <div className="w-full flex flex-col items-center justify-center gap-3 pt-6 pb-2 z-20">
                      <button
                        ...
                        className="w-full max-w-[200px] py-2.5 px-3 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white font-bold text-xs rounded-xl text-center cursor-pointer shadow-lg flex items-center justify-center gap-2 transition"
                      >
                        <span>📷 Open Camera</span>
                      </button>
                      <label 
                        htmlFor="front-picker"
                        className="w-full max-w-[200px] py-2 px-3 bg-slate-800 hover:bg-slate-700 text-slate-300 font-semibold text-xs rounded-xl text-center cursor-pointer border border-slate-700 flex items-center justify-center gap-2 transition"
                      >
              ...
              {isPro || scansLeft > 0 ? (
                ...
                >
                  {isScanning ? (
                    <span>ANALYZING CARD PIXELS & PSA CRITERIA...</span>
                  ) : (
                    <>
                      <span>RUN FULL PRE-GRADE INSPECTION</span>
                ...
                <button
                  onClick={() => setShowPaywall(true)}
                  className="w-full py-3.5 px-4 bg-gradient-to-r from-amber-500 to-red-500 text-white font-extrabold rounded-xl shadow-lg cursor-pointer"
                >
            ...
            {/* INSPECTION REPORT & MARKETPLACE LISTING SECTION */}
            {report && (
              <div className="bg-slate-900 border border-cyan-500/40 rounded-2xl p-5 shadow-2xl space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-700 relative">
                ...
                <button 
                  onClick={handleShare}
                  className="absolute -top-3 -right-3 bg-gradient-to-r from-emerald-500 to-teal-500 text-slate-950 font-black text-[10px] px-3 py-1.5 rounded-full shadow-[0_0_15px_#10b981] hover:scale-105 transition cursor-pointer uppercase tracking-widest"
                >
                  ↗ Share Certificate
                </button>
                ...
                <div className="bg-slate-950 border border-slate-800 rounded-xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-xs text-slate-300 mt-0.5">
                      Raw Market: <span className="font-semibold text-white">{report.rawVal}</span> → Graded Slab: <span className="font-bold text-emerald-400">{report.gradedVal}</span>
                    </p>
                  </div>
                  <span className="text-xs font-bold px-3 py-1.5 rounded-lg text-emerald-300 bg-emerald-950/80 border border-emerald-700/50">
                ...
                <div className="bg-gradient-to-br from-slate-950 to-cyan-950/40 border border-cyan-500/30 rounded-xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div>
                    <h4 className="text-sm font-bold text-white flex items-center gap-1.5">
                      <span>🏷️ List on Verified Marketplace</span>
                    </h4>
                    <p className="text-xs text-slate-400 mt-0.5">
                      Sell directly to buyers with this certified pre-grade report attached.
                    </p>
                  </div>
                  <div className="flex items-center gap-2 w-full sm:w-auto">
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder="Price"
                        className="w-24 bg-slate-900 border border-slate-700 pl-6 pr-2 py-1.5 text-xs text-white rounded-lg focus:outline-none focus:border-cyan-400"
                      />
                    </div>
                    <button
                      onClick={handleListToMarketplace}
                      disabled={isListed}
                      className="flex-1 sm:flex-none px-4 py-1.5 bg-emerald-500 hover:bg-emerald-400 disabled:bg-slate-800 text-slate-950 font-bold text-xs rounded-lg transition cursor-pointer shadow-lg shadow-emerald-500/20"
                    >
      ...
      {/* VIEW 2: VERIFIED MARKETPLACE VAULT */}
      {activeTab === 'marketplace' && (
        <main className="max-w-6xl mx-auto space-y-6 animate-in fade-in duration-500">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <button
              onClick={() => setActiveTab('scanner')}
              className="px-4 py-2 bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-bold text-xs rounded-xl shadow-lg cursor-pointer transition"
            >
              + Scan & List a Card
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {marketplace.map((card) => (
              <div 
                key={card.id}
                className="bg-slate-900 border border-slate-800 hover:border-cyan-500/50 rounded-2xl p-3.5 flex flex-col justify-between shadow-xl transition duration-200 group"
              >
                <div>
                  <div className="flex justify-between items-center text-[10px] font-bold mb-2">
                    ...
                    <span className="text-slate-400 font-mono text-[9px]">{card.verifiedCert}</span>
                  </div>
                  ...
                  <div className="w-full h-44 bg-slate-950 rounded-xl overflow-hidden flex items-center justify-center p-2 border border-slate-800/80 mb-3 relative">
                    ...
                    <span className="absolute bottom-2 right-2 bg-slate-900/90 text-cyan-300 font-mono text-[9px] px-1.5 py-0.5 rounded border border-slate-700">
                      Center: {card.centering}
                    </span>
                  </div>
                ...
                <div className="border-t border-slate-800 pt-3 mt-3 flex items-center justify-between">
                  <div>
                    ...
                    <span className="text-base font-black text-emerald-400">${card.askingPrice}</span>
                  </div>
                  <button
                    onClick={() => setSelectedCert(card)}
                    className="px-3 py-1.5 bg-slate-800 hover:bg-cyan-500 hover:text-slate-950 text-slate-200 font-bold text-xs rounded-lg border border-slate-700 transition cursor-pointer"
                  >
                    View Report
                  </button>
                </div>
          ...
          <div className="bg-slate-900 border border-cyan-500/50 max-w-md w-full rounded-2xl p-6 shadow-2xl relative space-y-4 animate-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[10px] font-mono text-cyan-400">{selectedCert.verifiedCert}</p>
              </div>
            </div>
            ...
            <div className="bg-slate-950 border border-slate-800 rounded-xl p-3 flex justify-between items-center">
              <div>
                ...
                <span className="text-lg font-black text-emerald-400">${selectedCert.askingPrice}</span>
              </div>
              <button
                onClick={() => alert(`Redirecting to checkout for ${selectedCert.title}...`)}
                className="px-4 py-2 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-xs rounded-xl shadow-lg shadow-emerald-500/25 transition cursor-pointer"
              >
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 max-w-md w-full rounded-2xl p-6 shadow-2xl relative text-center space-y-4">
            ...
            <p className="text-xs text-slate-400">
              Avoid submitting low grade candidates and unlock unlimited high-res optical scans & marketplace listings.
            </p>
            <button
              onClick={() => window.location.href = STRIPE_CHECKOUT_URL}
