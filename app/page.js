...
export default function Home() {
  ...
  const handlePublishListing = (e) => {
    ...
    setSalePrice('');
    alert(`Success! Card listed for $${parseFloat(salePrice).toFixed(2)} in the Verified Gallery.`);
  };
        ...
        <section className="lg:col-span-2 space-y-6">
              ...
              <div className="relative border-2 border-dashed border-slate-700 bg-slate-950 rounded-xl p-4 min-h-[300px] flex flex-col items-center justify-center overflow-hidden">
                ...
                {isScanning && (
                  <div className="absolute inset-0 bg-cyan-950/80 backdrop-blur-[2px] rounded-xl flex flex-col items-center justify-center border border-cyan-400 z-50 pointer-events-none transition-all duration-300">
                    ...
                    <span className="text-xs font-mono font-bold text-cyan-300 mt-3 text-center px-2">
                      {scanPhase}
                    </span>
                  </div>
                )}
          ...
          {report && (
            <div className="bg-slate-900 border border-cyan-500/40 rounded-2xl p-4 md:p-6 shadow-2xl space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-700 relative">
              ...
              <div className="bg-slate-950 border border-slate-800 rounded-xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-xs text-slate-300 mt-0.5">
                    Raw Price: <span className="font-semibold text-white">{report.rawVal}</span> →
                    Graded Value:{' '}
                    <span className="font-bold text-emerald-400">{report.gradedVal}</span>
                  </p>
                </div>
                <span
                  className={`text-xs font-bold px-3 py-1.5 rounded-lg text-center ${
                    report.grade?.includes('10')
              ...
              >
                <span>💰 List This Card for Sale ($ Escrow Protected)</span>
              </button>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="bg-cyan-950/30 p-3 rounded-xl border border-cyan-800/50">
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-[11px] text-cyan-400 font-bold uppercase tracking-wide">
                      Centering Ratio
                    </span>
                  ...
                  <p className="text-sm font-semibold text-white">
                    {report.centering?.measurements}
                  </p>
                  <p className="text-xs font-medium text-cyan-300 mt-0.5">
                    {report.centering?.ratio}
                  </p>
                  <p className="text-[10px] text-cyan-500/80 mt-1 italic">
                    {report.centering?.rubric}
                  </p>
                </div>
      ...
      {sellModalOpen && (
        <div className="fixed inset-0 bg-slate-950/85 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 max-w-md w-full rounded-2xl p-6 shadow-2xl relative">
            ...
            <p className="text-xs text-slate-400 mb-4">
              Your AI sub-grades and photo are automatically attached to this verified listing.
            </p>

            <div className="bg-slate-950 border border-slate-800 p-3 rounded-xl mb-4 flex items-center gap-3">
              {(frontImage || backImage) && (
                <img
              ...
              <div>
                <p className="text-xs font-bold text-white">
                  {report?.title || cardName || 'Graded Collectible'}
                </p>
                <p className="text-xs font-mono text-emerald-400">
                  {report?.grade} • Centering: {report?.centering?.score || '9.5'}
                </p>
              </div>
            </div>
            ...
            <form onSubmit={handlePublishListing} className="space-y-3">
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-300 uppercase mb-1">
                  Seller Handle / Store Name
                </label>
                <input
                  type="text"
                  placeholder="e.g. FricksVault"
                  value={sellerName}
